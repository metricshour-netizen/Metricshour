<template>
  <main class="max-w-4xl mx-auto px-4 py-10">
    <NuxtLink :to="`/crypto/${symbol.toLowerCase()}/`" class="text-gray-500 text-xs hover:text-gray-300 transition-colors mb-6 inline-block">
      ← {{ symbol.toUpperCase() }}
    </NuxtLink>
    ...
    <div v-if="!data" class="text-center py-20">
      ...
      <NuxtLink :to="`/crypto/${symbol.toLowerCase()}/`" class="text-orange-500 text-sm hover:text-orange-400 mt-3 inline-block">View {{ symbol.toUpperCase() }} →</NuxtLink>
    </div>
    ...
    <template v-else>
      ...
      <div class="mb-8">
        <div class="flex items-center gap-3 mb-3">
          ...
          <span class="text-[10px] text-gray-700">Updates every 2 minutes</span>
        </div>
      ...
      <div v-if="data.insight?.summary" class="relative border rounded-lg p-5 bg-[#0d1520] border-orange-900/50 mb-6">
        ...
        <div class="flex items-start gap-3">
          ...
          <div>
            <span class="text-[10px] font-bold uppercase tracking-widest text-orange-400 block mb-1.5">MetricsHour Intelligence</span>
      ...
      <div class="bg-[#111827] border border-[#1f2937] rounded-xl p-5 mb-6 flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p class="text-gray-500 text-xs mt-0.5">Price chart, market cap, 24h volume</p>
        </div>
        <NuxtLink :to="`/crypto/${symbol.toLowerCase()}/`" class="shrink-0 bg-orange-600 hover:bg-orange-500 text-white text-sm font-semibold px-5 py-2 rounded-lg transition-colors">
          See full analysis →
        </NuxtLink>
      </div>
...
const { data } = await useAsyncData(
  `moving-crypto-${symUp}`,
  () => $fetch<any>(`${apiBase}/api/crypto/${symUp}/moving`).catch((e) => {
...
if (!data.value) {
  await navigateTo(`/crypto/${symbol}/`, { redirectCode: 302 })
}
...
const metaDesc = `${name} is ${dir} ${pct}% today. Crypto market analysis and macro context from MetricsHour.`
...
useHead({
  ...
  meta: [
    ...
    { property: 'og:url', content: `https://metricshour.com/crypto/${symbol}/moving/` },
    { name: 'robots', content: data.value ? 'index, follow' : 'noindex, follow' },
  ],
  link: [{ rel: 'canonical', href: `https://metricshour.com/crypto/${symbol}/moving/` }],
})
</script>
